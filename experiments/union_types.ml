...
let rec typeof (ctx : (string * typ) list) expr =
  ...
  | TaggedValue (tag, TEnum (enum_name, cases), value) ->
    ...
    | Some _t ->
      failwith
        (Printf.sprintf "Tag %s given wrong type for enum %s" tag enum_name)
    | None ->
      failwith (Printf.sprintf "Tag %s not found in Enum %s" tag enum_name) )
  | TaggedValue (_name, TSym tsym, _value) ->
    typeof ctx (TaggedValue (_name, List.assoc tsym ctx, _value))
  | Let ([], then_expr) -> typeof ctx then_expr
  ...
  | Enum t -> t
  | Sym s -> List.assoc s ctx
  | TaggedValue (_, (TUnit | TInt), _) ->
    failwith "A Tagged value must be tagged with an Enum"


let with_stdlib expr =
  Let
    ( [ ("Bool", Enum (TEnum ("Bool", [ ("True", TUnit); ("False", TUnit) ])))
      ; ("Option", Enum (TEnum ("Option", [ ("Some", TInt); ("None", TUnit) ])))
      ]
    , expr )
