type source = int * char list (* index * rest *)

type 'a parseresult = (source * 'a, string) result

let is_spacing c = List.exists (fun x -> x = c) [ ' '; '\t'; '\n' ]

let incend position = (fst position, snd position + 1)

let rec strip_starting_spaces (source : source) : source =
  match source with
  | pos, x :: rest when is_spacing x -> strip_starting_spaces (pos + 1, rest)
  | _pos, x :: _rest when not (is_spacing x) -> source
  | pos, [] -> (pos, [])
...
let symbol (source : source) : Expr.expr parseresult =
  ...
  let rec inner (s : source) (is_escaping : bool) (acc : char list) =
    ...
    | false, (_pos, first :: _rest) when List.length acc = 0 && is_spacing first
      ->
    ...
    | false, (pos, first :: _rest)
      ...
      Ok
        (s, Expr.Sym ((start_pos, pos), Util.string_of_char_list (List.rev acc)))
    ...
    | true, (pos, first :: rest) when not (Expr.is_symbol_char first) ->
      inner (pos + 1, rest) false (first :: acc)
    | true, (pos, []) ->
      Error (Util.str [ "escaping EOF. "; string_of_int pos ])
    ...
    | false, (_pos, []) ->
      Error "`symbol` got an empty string - can't parse that into a Sym"
...
let symbol_native_string =
  map symbol (function
      | Ok (state, Sym (_sym_pos, result)) -> Ok (state, result)
      | Ok ((index, _rest), _) ->
        Error
          (Util.str
             [ "Result of symbol_native_string not a Symbol - at position "
             ; string_of_int index
             ] )
      | Error e -> Error e )
...
let symbol_tests =
  [ ( "Vectors are not symbols"
    , symbol (0, Util.char_list "[ a b c]")
      = Error "Invalid symbol '[ a b c]' from src '[ a b c]'" )
  ; ( "little symbol"
    , symbol (0, Util.char_list "xyx") = Ok ((3, []), Sym ((0, 3), "xyx")) )
  ; ( "long symbol"
...
let quoted_symbol (source : source) =
  ...
  | start_pos, first :: _rest when first = '\'' ->
    let rec inner (s : source) (is_escaping : bool) acc =
      match (is_escaping, s) with
      | false, (pos, first :: rest) when List.length acc = 0 && first = '\'' ->
      ...
      | true, (pos, first :: rest) when first = '\'' ->
        inner (pos + 1, rest) false (first :: acc)
      | true, (pos, first :: rest) when first = '\\' ->
        inner (pos + 1, rest) false (first :: acc)
      | false, (pos, first :: rest) when first = '\\' ->
        inner (pos + 1, rest) true acc
      | false, (pos, first :: rest) -> inner (pos + 1, rest) false (first :: acc)
    ...
    in
    inner src_without_preceding_spacings false []
  | pos, [] ->
    Error
      (Util.str
         [ "Can't parse empty symbol. Somewhere around position: "
         ; string_of_int pos
         ] )
...
let string (source : source) : Expr.expr parseresult =
  ...
  | start_pos, first :: _rest when first = '"' ->
    let rec inner (s : source) (is_escaping : bool) (acc : char list) :
      ...
      | false, (pos, first :: rest) when first = '"' ->
        Ok
          ( (pos, rest)
          , String ((start_pos, pos), Util.string_of_char_list (List.rev acc))
          )
      | true, (pos, first :: rest) when first = '"' ->
      ...
      | true, (pos, []) ->
        Error
          (Util.str [ "escaping EOF?"; Util.string_of_pos (start_pos, pos) ])
      | _ ->
        failwith
          (Util.str
             [ "is_escaping: "
  ...
  | start_position, [] ->
    Error
      (Util.str [ "Can't parse empty String"; string_of_int start_position ])
  | _ -> Error "Strings must begin with a double-quote \"\"\""


let string_tests =
  [ ( "Strings are parsed"
    , string (0, Util.char_list "  \"Hello\"  ")
...
let literal (lit : string) (source : source) : unit parseresult =
  let lit_list : char list =
    snd (strip_starting_spaces (-1, Util.char_list lit))
  in
  let rec inner
      (rest_of_lit : char list) (curr_source : source) (acc : char list) =
    match (rest_of_lit, curr_source) with
    | lit_first :: lit_rest, (position, src_first :: src_rest)
      when lit_first = src_first ->
      inner lit_rest (position, src_rest) (src_first :: acc)
    | _lit_first :: _lit_rest, (_position, _src_first :: _src_rest) ->
      Error
        (Util.str
           [ "Source '"
           ; Util.string_of_char_list (snd source)
    ...
    | [], _ ->
      ...
      then Ok ((fst curr_source + List.length acc, snd curr_source), ())
      else failwith "Bug in literal"
    | _lit_first :: _lit_rest, (_position, []) -> Error "Source too short"
  in
  inner lit_list (strip_starting_spaces source) []


let literal_tests =
  [ ( "literally skip spacesrc = s"
    , literal "a" (0, Util.char_list "    aaa") = Ok ((5, [ 'a'; 'a' ]), ()) )
  ; ( "literally fail with honesty"
    , literal "[" (0, Util.char_list "#{")
...
let andThen p0 p1 (source : source) =
  ...
  | Ok ((rest_pos, rest), r0) ->
    ...
    | Ok ((end_pos, rest), r1) -> Ok ((end_pos, rest), (r0, r1))
    | Error e -> Error e )
  | Error e -> Error e
...
let left p =
  map p (function
      | Ok (source, (left, _right)) -> Ok (source, left)
      | Error e -> Error e )


let right p =
  map p (function
      | Ok (source, (_left, right)) -> Ok (source, right)
      | Error e -> Error e )


let n_or_more (total_n : int) (parse : 'source_code -> 'a parseresult) :
    'source_code -> 'a list parseresult =
  let rec inner n acc source =
    match parse source with
    | Ok (src_rest, res) -> inner (n + 1) (res :: acc) src_rest
    | Error e -> if n >= total_n then Ok (source, List.rev acc) else Error e
  in
  inner 0 []
...
let n_or_more_tests =
  [ ( "take 4 a"
    , n_or_more 4 (literal "a") (0, Util.char_list "   aaaaaa ")
      = Ok ((9, [ ' ' ]), [ (); (); (); (); (); () ]) )
  ]


let orElse parse0 parse1 (source : source) =
  match parse0 source with
  | Ok (rest, result0) -> Ok (rest, result0)
  | Error error0 ->
    ( match parse1 source with
    | Ok (rest, r1) -> Ok (rest, r1)
    | Error e1 ->
      Error
        (Util.str
           [ "orElse Error src: '"
...
let rec orElse_list parsers src =
  match parsers with
  | [ parse ] -> parse src
  | parse :: rest_of_parsers -> (orElse parse (orElse_list rest_of_parsers)) src
  | [] -> Error "orElse_list given no parsers"


let vector (expression : source -> Expr.expr parseresult) (source : source) :
    Expr.expr parseresult =
  (map
     (right
        (left
           (andThen
              (andThen (literal "[") (n_or_more 0 expression))
              (literal "]") ) ) )
     (function
       | Ok ((pos, src), list) ->
         Ok ((pos, src), Expr.Vector ((fst source, pos), list))
       | Error e -> Error e ) )
    source
...
let set (expr : source -> Expr.expr parseresult) (source : source) :
  ...
  (map
     (right
        (left
           (andThen (andThen (literal "#{") (n_or_more 0 expr)) (literal "}#")) ) )
     (Util.take_ok (fun ((end_pos, src), list) ->
          let pos : int = end_pos in
          ((pos, src), Expr.Set ((fst source, pos), list)) ) ) )
    source
...
let set_tests =
  [ ( "empty set"
    , set symbol (0, Util.char_list "#{}# ok")
      = Ok ((4, [ ' '; 'o'; 'k' ]), Set ((0, 4), [])) )
  ; ( "set of sets"
    , set (set symbol) (0, Util.char_list "#{#{}# #{}#}#")
      = Ok ((13, []), Set ((0, 13), [ Set ((2, 6), []); Set ((6, 11), []) ])) )
  ; ( "set of symbol"
    , set symbol (0, Util.char_list "#{hello}#")
      = Ok ((9, []), Set ((0, 9), [ Sym ((2, 7), "hello") ])) )
  ; ( "set of symbol or quoted_symbol"
    , set
        (orElse symbol quoted_symbol)
        (0, Util.char_list "#{'quoted sym' unquoted}# rest of src")
      = Ok
          ( (24, [ ' '; 'r'; 'e'; 's'; 't'; ' '; 'o'; 'f'; ' '; 's'; 'r'; 'c' ])
          , Set
              ( (0, 24)
              , [ Sym ((2, 13), "quoted sym"); Sym ((14, 22), "unquoted") ] ) )
    )
  ; ( "set of a quoted sym"
    , set quoted_symbol (0, Util.char_list "#{ 'sym' }# ")
      = Ok ((10, [ ' ' ]), Set ((0, 10), [ Sym ((3, 7), "sym") ])) )
  ]
...
let pattern expression : source -> 'a Expr.pattern parseresult =
  let symbol_pattern =
    map (orElse symbol quoted_symbol) (function
        ...
        | Error e -> Error e
        | _ -> failwith "you made it a result ;-)" )
  in
  let tag_pattern =
    map
      (andThen
         (andThen
            (andThen (literal "(") (orElse symbol quoted_symbol))
            expression )
         (literal ")") )
      (Util.take_ok (function
          | source, ((((), Expr.Sym (name_pos, name)), child), ()) ->
            (source, Expr.PTag (name_pos, name, child))
          | _ -> failwith "don't know hwo to handle this" ) )
  in
  orElse symbol_pattern tag_pattern


let lambda (expression : source -> Expr.expr parseresult) (source : source) :
    Expr.expr parseresult =
  let pattern = pattern expression in
  (map
     (andThen
        (andThen
           (andThen (literal "(") (literal "λ"))
           (n_or_more 1 (andThen pattern expression)) )
        (literal ")") )
     (function
       | Ok
           ( (new_pos, rest)
           , ((((), ()), (PSym (psym_pos, x), body0) :: arg_bodies_list), ()) )
         ->
         Ok
           ( (new_pos, rest)
           , Expr.Lam
               ( (fst source, new_pos)
                 (* TODO make this use the skipped spaces version *)
               , (PSym (psym_pos, x), body0) :: arg_bodies_list ) )
...
let rec undeepen
    (source : source) (patterns_exps : (Expr.expr list * Expr.expr) list) =
  (* BUG *)
  let inner : Expr.expr list * Expr.expr -> Expr.expr Expr.pattern * Expr.expr =
    function
    | [ Sym (psym_pos, first) ], e -> (PSym (psym_pos, first), e)
    | Sym (psym_pos, first) :: rest, e ->
      ( PSym (psym_pos, first)
      , ( match undeepen source [ (rest, e) ] with
        | _new_source_code, x -> x ) )
    | _ -> failwith "undeepen yourself brah"
  in
  (source, Lam ((fst source, fst source + 1), List.map inner patterns_exps))
...
let deep_lambda (expr : source -> Expr.expr parseresult) (source : source) =
  (map
     (andThen
        ...
        (literal ")") )
     (Util.take_ok (function
          | new_source_code, ((((), ()), patterns_exprs), ()) ->
          undeepen
            new_source_code
            (List.map
               (function
...
let unit source : Expr.expr parseresult =
  (map
     (andThen (literal "<") (literal ">"))
     (Util.take_ok (fun ((end_pos, rest), ((), ())) ->
          ((end_pos, rest), Expr.Unit (fst source, end_pos)) ) ) )
    source
...
let u8 =
  map
    (andThen
       (andThen (andThen (literal "(") (literal "u8")) symbol)
       (literal ")") )
    (function
      | Ok
          ( (src_end_pos, src_rest)
          , ((((), ()), Sym ((sym_start_pos, sym_end_pos), name)), ()) ) ->
        ( match int_of_string_opt name with
        | Some number ->
          Ok
            ( (src_end_pos, src_rest)
            , Expr.U8 ((sym_start_pos, sym_end_pos), number) )
        | None -> Error (Util.str [ "Failiig to parse u8: "; name ]) )
      | Error e -> Error e
      | _ -> failwith "nopez seniorez" )


let tuple (expression : source -> Expr.expr parseresult) source :
    Expr.expr parseresult =
  (map
     (andThen (andThen (literal "<") (n_or_more 1 expression)) (literal ">"))
     (Util.take_ok (fun ((end_pos, rest), (((), children), ())) ->
          ((end_pos, rest), Expr.Tuple ((fst source, end_pos), children)) ) ) )
    source
...
let annotation (expression : source -> Expr.expr parseresult) source :
  ...
  (map
     ...
     (Util.take_ok (fun ((end_pos, rest), (((((), ()), t), e), ())) ->
          ((end_pos, rest), Expr.Ann ((fst source, end_pos), t, e)) ) ) )
    source


(* Since this would match many things, ensure it's last in the orElse
   sequence *)
let application (expression : source -> Expr.expr parseresult) source :
    Expr.expr parseresult =
  (map
     (andThen (andThen (literal "(") (n_or_more 2 expression)) (literal ")"))
     (* PRETTY GOOD *)
     (Util.take_ok (fun ((pos, rest), (((), xs), ())) ->
          (* Forgive me *)
          let rec appise = function
            | [ x ] -> x
            | x :: xs -> Expr.App ((fst source, pos), appise xs, x)
            | [] -> failwith "Go app yourself"
          in
          ((pos, rest), appise (List.rev xs)) ) ) )
    source
...
let let_expr expression source : Expr.expr parseresult =
  (map
     (andThen
        (andThen
           (andThen
              (andThen (literal "(") (literal "let"))
              (andThen (pattern expression) expression) )
           expression )
        (literal ")") )
     (Util.take_ok (function
         | ( (end_index, rest)
           , (((((), ()), (Expr.PSym (_psym_pos, name), definition)), body), ())
           ) ->
           ( (end_index, rest)
...
let dict expression source =
  (map
     (andThen
        (andThen (literal "{") (n_or_more 0 (andThen expression expression)))
        (literal "}") )
     (Util.take_ok (fun ((pos, rest), (((), keys_and_vals), ())) ->
          ((pos, rest), Expr.Dict ((fst source, pos), keys_and_vals)) ) ) )
    source
...
let parse_output_exe source =
  (map
     (andThen
        (andThen
           (orElse (literal "--output") (literal "-o"))
           symbol_native_string )
        (n_or_more 1 symbol_native_string) )
     (function
       | Ok ((_index, _rest), (((), _output_file), [])) ->
         Error
           "No input files given - add some source file name(s) at the end of \
            the command"
       | Ok ((index, rest), (((), output_file), input_files)) ->
         Ok
           ( (index, rest)
           , OutputExeToPath ((fst source, index), { input_files; output_file })
           )
       | Error e -> Error e ) )
    source
...
let parse_output_c source =
  (map
     (andThen
        (andThen
           (orElse (literal "--output-c") (literal "-oc"))
           symbol_native_string )
        (n_or_more 1 symbol_native_string) )
     (function
       | Ok ((_index, _rest), (((), _output_file), [])) ->
       ...
       | Ok ((index, rest), (((), output_file), input_files)) ->
         Ok
           ( (index, rest)
           , OutputCToPath ((fst source, index), { input_files; output_file })
           )
       | Error e -> Error e ) )
    source
...
let print_help (source : source) =
  (map
     (orElse_list [ literal "--help"; literal "-h" ])
     (function
       | Ok ((index, rest), ()) ->
         Ok ((index, rest), PrintHelp (fst source, index))
       | Error e -> Error e ) )
    source
...
let print_welcome source =
  (map (literal "") (function
      | Ok ((index, rest), ()) ->
        Ok ((index, rest), NoCommandArguments (fst source, index))
      | Error e -> Error e ) )
    source
...
let parse_args =
  map
    ...
    (Util.take_ok (fun ((end_pos, rest), (_, cmd)) -> ((end_pos, rest), cmd)))
  ...
  [ ( "Output C file"
    , let src = "platoc --output-c myfile.c mysource.plato" in
      ...
      Ok
        ...
        , OutputCToPath
            ( pos
            , { input_files = [ "mysource.plato" ]; output_file = "myfile.c" }
            ) ) )
  ]
