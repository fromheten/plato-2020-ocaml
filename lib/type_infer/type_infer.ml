...
let rec typeof_exn
    ...
    (non_generic : TVSet.t) : Type.Type.typ =
  match expr with
  ...
  | Expr.App (pos, fn, arg) ->
    ...
    let result_type_param =
      Type.Type.TyVar (pos, Type.TypeVariable.create gensym_state)
    in
...
and occurs_in (t : Type.TypeVariable.t) types =
...
let type_infer_tests =
  [ (let actually =
     ...
     ( Printf.sprintf
         ...
         (Type.Type.to_string (Type.new_gensym_state ()) actually)
     , actually
       = Type.Type.TyOp
           ( negpos
           , "->"
